  ...
  async execute(interaction, api) {
    try {
      ...
      while (hasMoreProducts && page <= 50) {
        try {
          const products = await api.get(`sellhub/products?shop_id=${api.shopId}&limit=100&page=${page}`);
      ...
      try {
        ...
        while (hasMoreInvoices && invPage <= 50) {
          try {
            const invoices = await api.get(`sellhub/invoices?shop_id=${api.shopId}&limit=250&page=${invPage}`);
