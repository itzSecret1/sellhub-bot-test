...
let config = {
  ...
  // Support both SH_* (SellHub) and SA_* (SellAuth) for migration compatibility
  SH_API_KEY: process.env.SH_API_KEY || process.env.SA_API_KEY || '',
  SH_SHOP_ID: process.env.SH_SHOP_ID || process.env.SA_SHOP_ID || '', // Optional - will be auto-detected from API if not provided
